...
function syncBestBidBestAsk(){
	for(let pr of (feed.getProductsPairs(ExchangeCodes.BITSTAMP)) ){
		for(let symbol of pr.symbols){
			// best bid best ask
			feed.getBestBidBestAsk(ExchangeCodes.BITSTAMP, {productID:symbol}).then(best_bid_best_ask=>{
...
function syncTicker(){
	for(let pr of (feed.getProductsPairs(ExchangeCodes.BITSTAMP)) ){
		for(let symbol of pr.symbols){
			// ticker 
			feed.getTicker(ExchangeCodes.BITSTAMP, {productID:symbol}).then(tick=>{
...
function syncTrades(){
	for(let pr of (feed.getProductsPairs(ExchangeCodes.BITSTAMP)) ){
		for(let symbol of pr.symbols){
			// Trades 
			feed.getAllTrades(ExchangeCodes.BITSTAMP, {productID:symbol, interval:'hour'}).then(trades=>{
